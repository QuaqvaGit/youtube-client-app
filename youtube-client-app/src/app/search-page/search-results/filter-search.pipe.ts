...
export default class FilterSearchPipe implements PipeTransform {
  ...
  private filterCallback: (item: SearchItem, searchValue: string) => boolean = (item, searchValue) => item.snippet.title.toLowerCase().includes(searchValue) ||
    item.snippet.tags
  ...
  transform(items: SearchItem[], ...args: string[]): SearchItem[] {
    ...
    return items.filter(
      (item) => this.filterCallback(item, searchValue)
    );
  }
}
